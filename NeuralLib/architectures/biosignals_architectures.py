...
class GRUseq2seq(Architecture):
    def __init__(
        ...
        :param bidir_per_layer: List indicating bidirectionality per layer (overrides bidirectional if provided).
            ...
            Broadcasting rules:
            - If None: all layers use `bidirectional`.
            - If len(bidir_per_layer) < n_layers:
                the list is extended to length `n_layers` by repeating its last
                value for the remaining layers.
            - If len(bidir_per_layer) > n_layers:
                only the first `n_layers` values are used.
